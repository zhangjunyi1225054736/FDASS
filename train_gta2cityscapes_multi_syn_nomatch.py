...
SNAPSHOT_DIR = '/data2/zhangjunyi/snapshots/'
WEIGHT_DECAY = 0.0005
...
def main():
    ...
    if args.model == 'DeepLab':
        ...
        if args.restore_from[:4] == 'http' :
            ...
            # saved_state_dict = torch.load('snapshots/GTA2Cityscapes_multi_wn/GTA5_10000.pth')
    ...
    # init D
    model_D1 = FCDiscriminator(num_classes=args.num_classes, match=False)
    #model_D2 = model_D1
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            fake1_D = pred1_target_D
            fake2_D = pred2_target_D
            ...
            real1_D = pred1_D
            real2_D = pred2_D
            fake1_D_ = pred1_target_D
            fake2_D_ = pred2_target_D

            D_out1_real = model_D1(real1_D)
            D_out2_real = model_D2(real2_D)
            D_out1_fake = model_D1(fake1_D_)
            D_out2_fake = model_D2(fake2_D_)
            ...
            loss_D3 = bce_loss(D_out1_fake,
                              Variable(torch.FloatTensor(D_out1_fake.data.size()).fill_(target_label)).cuda(gpu_id_1))

            loss_D4 = bce_loss(D_out2_fake,
                               Variable(torch.FloatTensor(D_out2_fake.data.size()).fill_(target_label)).cuda(gpu_id_1))
